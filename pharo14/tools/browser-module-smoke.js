...
renderDisplay.sdlEventQueue = [];
Squeak.FFIEmulation.enqueueSDLKeyDownFromBrowserEvent(renderDisplay, { code: "KeyZ", key: "Z", shiftKey: true, ctrlKey: false, altKey: false, metaKey: false, repeat: false, timeStamp: 4242 });
Squeak.FFIEmulation.enqueueSDLTextInputFromBrowserEvent(renderDisplay, "ž", 4243);
Squeak.FFIEmulation.enqueueSDLKeyUpFromBrowserEvent(renderDisplay, { code: "KeyZ", key: "Z", shiftKey: false, ctrlKey: false, altKey: false, metaKey: false, timeStamp: 4244 });
const keyEventAddress = renderPrim.ffiMakeStExternalAddress();
keyEventAddress.jsData = new ArrayBuffer(56);
r = callSDL(renderPrim, "SDL_PollEvent", intType, [keyEventAddress], pointerType);
assert.strictEqual(r.ok, true, "browser render smoke polls direct SDL keydown");
let keyEventView = new DataView(keyEventAddress.jsData);
assert.strictEqual(keyEventView.getUint32(0, true), 0x300, "browser render smoke direct keydown uses SDL_KEYDOWN");
assert.strictEqual(keyEventView.getInt32(16, true), 29, "browser render smoke maps DOM KeyZ to SDL scancode Z");
assert.strictEqual(keyEventView.getInt32(20, true), 122, "browser render smoke maps shifted letter key symbol to lowercase SDL keycode");
assert.strictEqual(keyEventView.getUint16(24, true) & 0x0003, 0x0003, "browser render smoke preserves Shift modifier bits");
r = callSDL(renderPrim, "SDL_PollEvent", intType, [keyEventAddress], pointerType);
assert.strictEqual(r.ok, true, "browser render smoke polls direct SDL text input");
keyEventView = new DataView(keyEventAddress.jsData);
assert.strictEqual(keyEventView.getUint32(0, true), 0x303, "browser render smoke direct text uses SDL_TEXTINPUT");
assert.strictEqual(keyEventView.getUint8(12), 0xC5, "browser render smoke stores UTF-8 byte 1 for ž");
assert.strictEqual(keyEventView.getUint8(13), 0xBE, "browser render smoke stores UTF-8 byte 2 for ž");
r = callSDL(renderPrim, "SDL_PollEvent", intType, [keyEventAddress], pointerType);
assert.strictEqual(r.ok, true, "browser render smoke polls direct SDL keyup");
keyEventView = new DataView(keyEventAddress.jsData);
assert.strictEqual(keyEventView.getUint32(0, true), 0x301, "browser render smoke direct keyup uses SDL_KEYUP");
assert.strictEqual(Squeak.externalModules["libSDL2-2.0"].keyboardState[29], 0, "browser render smoke clears the released SDL scancode state");
