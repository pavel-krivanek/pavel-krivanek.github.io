...
const resolutionY = 30;
const screenWidth = 1040;
const screenHeight = 780;

const lineHeight = 20;
const linesSpacing = 10;

const margin_top = 0//(screenHeight-(resolutionY*(ypx+linesSpacing)))/2;
const margin_left = 0//(screenWidth-(resolutionX*xpx))/2;

var pageScrollSpeed = 0;
var x = 0 * xpx;
var y = 25 * ypx;
var maxY = y;
var minY = y;
var vmid = 25 * ypx //$(window).height() / 2;
var hmid = 0 // $(window).width() / 2;
var voffset = {};
var brokenness = 15;
...
//$('.output').css('padding-top', margin_top + 'px');


//$('.char').css('width', resolutionX*xpx + 'px');
...
//     $(where).text(content);
    ...
    lines.forEach((line, index) => {
        ...
        $(where).append(`<pre class="char">${line}${cursor}</pre>`);
    });
    ...
    var base_colour = '255,0,0';

    if (black_height > 0) {
        // Output the (possibly partial) character in black
        $(where).append('<div class="char" style="position: absolute; ' + vpos + hpos + ' color: #ffd00e; ' + black_height_style + '">' + c + '</div>');
...
function setCursorPosition() {
    $(function() {
        $('.cursor').animate({
           top: (((lineHeight + linesSpacing)*(resolutionY-1)) + margin_top ) + 'px',
           left: (x+ margin_left-10 ) + 'px',
        }, {
