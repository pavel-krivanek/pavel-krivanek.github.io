...
The printer mechanism stores each strike at its mechanical raster coordinate plus normalized random samples. Dot darkness, per-dot darkness variability, diameter, positional offset and shape are renderer properties. The notch setting is different: it controls the physical width of the periodic fine-encoder pulse and therefore changes future 8255 timing and strike coordinates. Existing marks are never repositioned when notch width changes. `100%` dot size means a diameter equal to one nominal raster pitch. The current defaults are white paper, dark carbon ink, 75% darkness, 33% darkness variability, 110% dot size, 20% mechanical notch width and ±11% random positional offset, with randomized dot shapes enabled. Paper and ink are arbitrary RGB colours selected by the UI; both are renderer-only properties, so changing either immediately recolours the retained page without affecting printer timing or stored strikes.
...
A separate registration defect became clear with `LPRINT "I"`. At authentic speed, the BT1 driver fires the two stem dots at approximately `4.476/5.476` while moving away from home and `5.944/6.944` while returning. In the BT1/Desktop convention, return interval `N` corresponds to visual column `N-1`. The correction preserves that mapping and the remaining approximately half-pitch difference arising from the finite PA5 pulse and instruction timing.

Busy soft's C-2 routines use the opposite completed-cell convention. Their outward scan belongs to `N-1`, while the return scan belongs to `N`. Applying BT1's return correction globally placed alternating Busy raster rows about 2.14 pitches apart even though all intended dots were present. Encoder-cell numbering is now selected with the connection/driver profile: C-2 uses the Busy convention and the other profiles retain the BT1/Desktop convention. The Busy regression compares each strike with the raster prepared in memory, so this error can no longer pass as a mere dot-count success.
...
### High-resolution timing used by Busy soft

Advanced BT-100 software does not place every strike on a complete encoder pitch. It synchronizes to a fine or coarse optical edge and then uses calibrated Z80 delays to fire at fractional positions. The printer model therefore stores carriage and paper motion on a 65,536-microstep fixed-point grid per optical pitch. Fine and coarse inputs are calculated from that independent mechanical coordinate rather than from the clipped preview position.

The previous integration also limited every update to 20,000 T-states. A longer delay loop consequently lost mechanical travel and changed the phase between the encoder and needle. The fixed-point integrator now consumes the entire elapsed emulated interval. The standard speed calibration is still six seconds for the original 480-position scan, while the retained paper surface is 512 columns wide for software that extends the normal raster. Sixteen pitches of run-out at either end keep the optical phase changing while a driver performs final alignment outside the nominal line.

The Busy soft regression loads the original BT-BCS machine-code driver and BT-TST text, selects C-2, and prints its bidirectional miniature sample at authentic speed. It now compares every physical strike against the eight 128-byte raster planes generated by the driver. Each set bit represents a half-pitch position; the maximum observed error is about 0.211 optical pitch. The test also verifies a stopped motor and a successful return home.
...
## Mechanical notch width (v039)

The UI notch value is now passed to `BT100Printer.setNotchSize()`. It defines the optical cut-out width as 1–99% of one pitch; the asserted electrical interval is its complement under the preserved BT-100 input polarity. The paper encoder retains an independent fixed pulse width. Because the driver synchronizes against these edges, the setting naturally affects bidirectional registration and calibrated delay placement during future printing. `drawPrintedDot()` no longer adds a direction-dependent offset, and changing the control does not invalidate or redraw retained marks.
